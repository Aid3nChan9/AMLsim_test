"""
AML Detection Model v11.0
- 混合采样 SMOTETomek
- 集成学习：随机森林 + XGBoost（可配置权重）
- 阈值优化：F1.5 最大化（平衡召回率与精确率）
- 无高级特征（避免特征空间不一致）
- 无静默账户后处理（让模型基于图特征自行判断）
使用方法：
    python aml_model_v11.py --data_dir <路径> [--rf_weight 0.5] [--xgb_weight 0.5]
"""
...
def find_threshold_fbeta(y_val, y_val_proba, beta=1.5):
    ...
    for th in thresholds:
        ...
        if fb > best_fbeta:
            best_fbeta = fb
            best_th = th
    y_pred_final = (y_val_proba >= best_th).astype(int)
    final_rec = recall_score(y_val, y_pred_final)
    final_prec = precision_score(y_val, y_pred_final)
    print(f"F{beta} 最大化策略 → 最佳阈值={best_th:.2f} (F{beta}={best_fbeta:.4f}, 召回={final_rec:.4f}, 精确={final_prec:.4f})")
    return best_th

# ===================== 4. 主流程 =====================
def run(data_dir, rf_weight=0.5, xgb_weight=0.5):
    print(f"\n{'='*60}")
    print(f"AML Detection Model v11.0 (混合采样 + F1.5 阈值 + 集成)")
    ...
    plt.savefig('confusion_matrix_v11.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    if not USE_GPU_RF:
        ...
        plt.title('随机森林特征重要性 (v11)')
        plt.barh(range(len(indices)), importances[indices], align='center')
        plt.yticks(range(len(indices)), [feature_cols[i] for i in indices])
        plt.gca().invert_yaxis()
        plt.tight_layout()
        plt.savefig('feature_importance_v11.png', dpi=150, bbox_inches='tight')
        plt.close()
        print("特征重要性图已保存: feature_importance_v11.png")
    ...
    if not USE_GPU_RF:
        ...
        plt.savefig('shap_summary_v11.png', dpi=150, bbox_inches='tight')
        plt.close()
        ...
        if pos.any():
            ...
            plt.savefig('shap_waterfall_v11.png', dpi=150, bbox_inches='tight')
            plt.close()
            print("SHAP 瀑布图已保存: shap_waterfall_v11.png")
        else:
    ...
    suspicious_list.to_csv('suspicious_accounts_v11.csv', index=False)
    print(f"\n可疑账户名单已保存: suspicious_accounts_v11.csv")
    print(f"共识别出 {len(suspicious_list)} 个可疑账户 (其中真实可疑 {suspicious_list['TRUE_LABEL'].sum()} 个)")
    
    return rf, xgb_model, best_th

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="AML Detection Model v11.0")
