...
def calculate_advanced_features(trans, accounts, G=None):
    ...
    for acc in accounts['ACCOUNT_ID'].unique():
        ...
        if inflow > 0:
            turnover_ratios[acc] = outflow / inflow
        else:
...
def extract_features(trans, accounts):
    ...
    return X, y, feature_cols, account_features[['ACCOUNT_ID']]  # 返回账户ID用于输出名单

def find_threshold_by_recall(y_val, y_val_proba, recall_target):
    """在验证集上找到达到目标召回率的最低阈值（最大化精确率）"""
...
def run(data_dir, recall_target=0.95):
    """主流程"""
    print(f"\n{'='*60}")
    print(f"AML Detection Model v5.0")
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', 
                xticklabels=['正常','可疑'], yticklabels=['正常','可疑'])
    plt.xlabel('预测')
    plt.ylabel('真实')
    plt.title(f'测试集混淆矩阵(阈值={best_th:.2f})(总样本={total})')
    plt.savefig('confusion_matrix_v5.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("混淆矩阵已保存: confusion_matrix_v5.png")
    ...
    plt.tight_layout()
    plt.savefig('feature_importance_v5.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("特征重要性图已保存: feature_importance_v5.png")
    ...
    plt.savefig('shap_summary_v5.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("SHAP 汇总图已保存: shap_summary_v5.png")
    
    # 瀑布图（只要真实可疑就画，不要求预测正确）
    ...
    if pos.any():
        ...
        plt.savefig('shap_waterfall_v5.png', dpi=150, bbox_inches='tight')
        plt.close()
        print("SHAP 瀑布图已保存: shap_waterfall_v5.png")
    else:
        print("样本子集中无真实可疑账户，跳过瀑布图")
    
    # 输出可疑账户名单（测试集所有账户的风险概率和预测标签）
    # 获取测试集对应的账户ID
    ...
    suspicious_list.to_csv('suspicious_accounts_v5.csv', index=False)
    print(f"\n可疑账户名单已保存: suspicious_accounts_v5.csv")
    print(f"共识别出 {len(suspicious_list)} 个可疑账户 (其中真实可疑 {suspicious_list['TRUE_LABEL'].sum()} 个)")
    
    return best_rf, best_th

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="AML Detection Model v5.0")
