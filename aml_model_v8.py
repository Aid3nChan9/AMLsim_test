...
使用方法：
    python aml_model_v8.py --data_dir <路径> [--rf_weight 0.3] [--xgb_weight 0.7]
"""
...
# ===================== 3. 模型训练与集成 =====================
def find_threshold_fbeta(y_val, y_val_proba, beta=0.5):
    """最大化 F-beta 分数，beta < 1 时精确率权重大"""
...
def run(data_dir, rf_weight=0.3, xgb_weight=0.7):
    print(f"\n{'='*60}")
    print(f"AML Detection Model v8.0 (集成学习 + F0.5 阈值 + 混合采样)")
    ...
    # 优化阈值（F0.5 最大化）
    best_th = find_threshold_fbeta(y_val, y_val_proba, beta=0.5)
    ...
    plt.savefig('confusion_matrix_v8.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    if not USE_GPU_RF:
        ...
        plt.title('随机森林特征重要性 (v8)')
        plt.barh(range(len(indices)), importances[indices], align='center')
        plt.yticks(range(len(indices)), [feature_cols[i] for i in indices])
        plt.gca().invert_yaxis()
        plt.tight_layout()
        plt.savefig('feature_importance_v8.png', dpi=150, bbox_inches='tight')
        plt.close()
        print("特征重要性图已保存: feature_importance_v8.png")
    ...
    if not USE_GPU_RF:
        ...
        plt.savefig('shap_summary_v8.png', dpi=150, bbox_inches='tight')
        plt.close()
        # 瀑布图
        ...
        if pos.any():
            ...
            plt.savefig('shap_waterfall_v8.png', dpi=150, bbox_inches='tight')
            plt.close()
            print("SHAP 瀑布图已保存: shap_waterfall_v8.png")
        else:
    ...
    suspicious_list.to_csv('suspicious_accounts_v8.csv', index=False)
    print(f"\n可疑账户名单已保存: suspicious_accounts_v8.csv")
    print(f"共识别出 {len(suspicious_list)} 个可疑账户 (其中真实可疑 {suspicious_list['TRUE_LABEL'].sum()} 个)")
    
    return rf, xgb_model, best_th

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="AML Detection Model v8.0")
