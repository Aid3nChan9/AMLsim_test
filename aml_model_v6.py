"""
AML Detection Model v6.0
支持两种阈值优化策略：
  - recall  : 固定召回率目标（最大化精确率）
  - fbeta   : 最大化 F-beta 分数（beta>1 侧重召回率）
输出可疑账户名单（CSV）、混淆矩阵、特征重要性、SHAP 图。
使用方法：
    python aml_model_v6.py --data_dir <路径> --strategy recall --recall_target 0.95
    python aml_model_v6.py --data_dir <路径> --strategy fbeta --beta 2.0
"""
...
def calculate_advanced_features(trans, accounts, G=None):
    ...
    for acc in accounts['ACCOUNT_ID'].unique():
        ...
        turnover_ratios[acc] = outflow / inflow if inflow > 0 else 0
    ...
    all_avg_lengths = {node: avg_path_lengths.get(node, median_length) for node in nodes}
...
def find_threshold(y_val, y_val_proba, strategy, recall_target=None, beta=2.0):
    thresholds = np.linspace(0.01, 0.99, 100)
    if strategy == 'recall':
        assert recall_target is not None, "recall 策略需要提供 recall_target"
        best_th = 0.5
        best_precision = 0
        ...
        for th in thresholds:
            ...
            if fb > best_fbeta:
                best_fbeta = fb
                best_th = th
        print(f"F{beta} 最大化策略 → 最佳阈值={best_th:.2f} (F{beta}={best_fbeta:.4f})")
        return best_th
    else:
...
def run(data_dir, strategy='recall', recall_target=0.95, beta=2.0):
    print(f"\n{'='*60}")
    print(f"AML Detection Model v6.0")
    ...
    X, y, feature_cols, account_ids_df = extract_features(trans, accounts)
    ...
    best_th = find_threshold(y_val, y_val_proba, strategy, recall_target, beta)
    ...
    plt.tight_layout()
    plt.savefig('feature_importance_v6.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig('shap_summary_v6.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    if pos.any():
        ...
        plt.savefig('shap_waterfall_v6.png', dpi=150, bbox_inches='tight')
        plt.close()
        print("SHAP 瀑布图已保存: shap_waterfall_v6.png")
    else:
        print("样本子集中无真实可疑账户，跳过瀑布图")

    # 输出可疑账户名单
    test_account_ids = account_ids_df.loc[X_test.index, 'ACCOUNT_ID'].values
    ...
    suspicious_list.to_csv('suspicious_accounts_v6.csv', index=False)
    print(f"\n可疑账户名单已保存: suspicious_accounts_v6.csv")
    print(f"共识别出 {len(suspicious_list)} 个可疑账户 (其中真实可疑 {suspicious_list['TRUE_LABEL'].sum()} 个)")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="AML Detection Model v6.0")
    ...
    parser.add_argument('--recall_target', type=float, default=0.95, help='召回率目标 (仅recall策略)')
    parser.add_argument('--beta', type=float, default=2.0, help='F-beta 中的 beta 参数 (仅fbeta策略)')
